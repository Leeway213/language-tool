export function doIntervalsOverlap (interval1: [number, number], interval2: [number, number]) {
...
export function isClose (a: number, b: number, relTol = 1e-14, abs_tol = 0.0) {
...
export function sortCompareEntriesByTime (x: any[], y: any[]) {
  return x[0] - y[0];
}
 ...
 */
export function entryListToTree (entryList: any[]) {
...
export function recEntryListToTree (entryList: any[]) {
 ...
 */
export function findIntervalAtTime (time: number, rootNode: any) {
 ...
 */
export function findPointAtTime (time: any, rootNode: any, findClosest = false) {
  let currNode = rootNode;
  let matchNode = null;
