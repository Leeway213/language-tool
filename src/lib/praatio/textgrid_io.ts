 ...
 * @ignore
 */
function extendedSplit (str: string, separator: string , max: number) {
  const out = [];
  let index = 0;
  let next;
...
function findAllSubstrings (sourceStr: string | any[], subStr: string) {
  const indexList = [];
...
function fetchRow (dataStr: string, searchStr: string | any[], index: number) {
  const startIndex = dataStr.indexOf(searchStr as any, index) + searchStr.length;
...
function parseNormalTextgrid (data: string) {
  // Toss header
  ...
  for (let i = 0; i < tierList.length; i++) {
    ...
    if (tierType === INTERVAL_TIER) {
      let timeStartI: any = null;
      let timeEndI:any = null;
      ...
      let timePointI: any = null;
      let timePoint: any = null;
      while (true) {
...
function parseShortTextgrid (data: string | any[]) {
  const indexList = [];
  ...
  indexList.sort(function (x: any, y: any) {
    return x[0] - y[0];
  });
  ...
  const header: any = data.slice(0, tupleList[0][0]);
  ...
  for (let i = 0; i < tupleList.length; i++) {
    ...
    const tierData: any = data.slice(blockStartI, blockEndI);

    const metaStartI: any = fetchRow(tierData as any, '', 0)[1];

    // Tier meta-information
    const [tierName, tierNameEndI] = fetchRow(tierData as any, '', metaStartI);
    let [tierStartTime, tierStartTimeI] = fetchRow(tierData, '', tierNameEndI as any);
    let [tierEndTime, tierEndTimeI] = fetchRow(tierData, '', tierStartTimeI as any);
    let startTimeI = fetchRow(tierData, '', tierEndTimeI as any)[1];

    tierStartTime = parseFloat(tierStartTime as any);
    tierEndTime = parseFloat(tierEndTime as any);

    // Tier entry data
    let startTime: any = null;
    let endTime: any = null;
    let label: any = null;
    // let tierType: any = null;
    let endTimeI: any = null;
    let labelI: any = null;

    const entryList = [];
    if (isInterval === true) {
      while (true) {
        [startTime, endTimeI] = fetchRow(tierData, '', startTimeI as any);
        if (endTimeI === -1) break;
      ...
      tier = new IntervalTier(tierName as any, entryList, tierStartTime, tierEndTime);
    } else {
      while (true) {
        [startTime, labelI] = fetchRow(tierData, '', startTimeI as any);
        if (labelI === -1) break;
 ...
 * @ignore
 */
function fillInBlanks (tier: IntervalTier, blankLabel = '', startTime: any = null, endTime: any = null) {
  ...
  return copyTier(tier as any, { entryList: newEntryList });
}
 ...
 * @ignore
 */
function tierToText (tier: { tierType: string; name: any; minTimestamp: any; maxTimestamp: any; entryList: string | any[]; }) {
  let text = ''
  text += `"${tier.tierType}"\n`;
  text += `"${tier.name}"\n`;
 ...
 * @ignore
 */
function removeUltrashortIntervals (tier: { entryList: any; name?: any; minTimestamp?: any; maxTimestamp?: any; constructor?: new (arg0: any, arg1: any, arg2: any, arg3: any) => any; }, minLength: number, minTimestamp: null) {
  // First, remove tiny intervals
  const newEntryList: any[][] = [];
  let j = 0;
  for (let i = 0; i < tier.entryList.length; i++) {
 ...
 * @return {text}
 */
function serializeTextgridToCsv (tg: { tierNameList: any; tierDict: any; }, pivotTierName: string | number, tierNameList: any = null, includeHeader = true) {
 ...
 * @param {number} [minTimestamp: any = null] -- the minTimestamp of the saved Textgrid; if None, use whatever is defined in the Textgrid object.  If minTimestamp is larger than timestamps in your textgrid, an exception will be thrown.
 * @param {number} [maxTimestamp: any = null] -- the maxTimestamp of the saved Textgrid; if None, use whatever is defined in the Textgrid object.  If maxTimestamp is larger than timestamps in your textgrid, an exception will be thrown.
 ...
 */
function serializeTextgrid (tg: { minTimestamp: null; maxTimestamp: null; }, minimumIntervalLength = MIN_INTERVAL_LENGTH, minTimestamp: any = null, maxTimestamp: any = null, useShortForm = false) {
  if (minTimestamp === null) minTimestamp = tg.minTimestamp;
  if (maxTimestamp === null) maxTimestamp = tg.maxTimestamp;

  const outputTg = prepTgForSaving(tg as any, minimumIntervalLength, minTimestamp, maxTimestamp);
...
function tgToShortTextForm (tg: { tierNameList: string | any[]; tierDict: { [x: string]: any; }; }, minTimestamp: null, maxTimestamp: null) {
  let outputTxt = '';
...
function tgToLongTextForm (tg: { tierNameList: string | any[]; tierDict: { [x: string]: any; }; }, minTimestamp: null, maxTimestamp: null) {
  const tab = ' '.repeat(4);
 ...
 */
function prepTgForSaving (tg: { minTimestamp: null; maxTimestamp: null; tierNameList: string | any[]; tierDict: { [x: string]: any[]; }; }, minimumIntervalLength = MIN_INTERVAL_LENGTH, minTimestamp: any = null, maxTimestamp: any = null) {
  if (minTimestamp === null) minTimestamp = tg.minTimestamp;
  if (maxTimestamp === null) maxTimestamp = tg.maxTimestamp;
  ...
  for (let i = 0; i < tg.tierNameList.length; i++) {
    ...
    if (tier instanceof IntervalTier) {
      ...
      if (minimumIntervalLength !== null) {
        tier = removeUltrashortIntervals(tier as any, minimumIntervalLength, minTimestamp);
      }
 ...
 * @return {Textgrid}
 */
function parseTextgrid (text: string, readRaw = false) {
  text = decodeBuffer(text as any) as any;
  text = text.replace(/\r\n/g, '\n');
...
function removeBlanks (tier: { entryList: any; name?: any; minTimestamp?: any; maxTimestamp?: any; constructor?: new (arg0: any, arg1: any, arg2: any, arg3: any) => any; }) {
  const entryList = [];
  for (let i = 0; i < tier.entryList.length; i++) {
  ...
  return copyTier(tier as any, { entryList: entryList });
}
